...
#ifdef __CUDACC__
// nvcc's device front-end lacks C++20 concepts; drop the constraint for device
// (callers always pass unsigned char / std::byte). Host keeps the strict concept.
#define ByteType typename
#else
#include <concepts>
template <typename B>
