...
static constexpr unsigned int MAX_PQ_PUBKEYS_PER_MULTISIG{8};

// Maximum size of a single execution-stack element or push value in P2MR scripts.
static constexpr unsigned int MAX_P2MR_ELEMENT_SIZE = 10000;

// Maximum leaf script size for P2MR (witness v2) spends.
// Raised from 10000 to 11000 to support 8-of-8 ML-DSA multisig
// (8 × 1312-byte pubkeys = 10496 bytes + opcodes).
static constexpr unsigned int MAX_P2MR_SCRIPT_SIZE = 11000;

// Maximum aggregate bytes across stack + altstack during P2MR execution.
static constexpr uint64_t MAX_P2MR_STACK_BYTES = 1'000'000;
...
enum opcodetype
{
    ...
    OP_CHECKSIGADD_MLDSA = 0xbe,
    OP_CHECKSIGADD_SLHDSA = 0xbf,
    // Reserved P2MR soft-fork slots. These intentionally remain OP_SUCCESSx
    // today so Falcon-style activation can become a future soft fork.
    OP_CHECKSIG_FALCON = 0xc0,
    OP_CHECKSIGADD_FALCON = 0xc1,
    OP_CHECKSIGFROMSTACK_FALCON = 0xc2,

    OP_INVALIDOPCODE = 0xff,
};

// Maximum value that an opcode can be
static const unsigned int MAX_OPCODE = OP_CHECKSIGFROMSTACK_FALCON;
