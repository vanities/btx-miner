...
public:
    ...
    auto GetPos() const requires requires { GetStream().GetPos(); }
    {
        return GetStream().GetPos();
    }
    template <typename Pos>
    bool SetPos(Pos pos) requires requires { GetStream().SetPos(pos); }
    {
        return GetStream().SetPos(pos);
    }
    bool Rewind(std::optional<size_t> n = std::nullopt)
        requires requires { GetStream().Rewind(n); }
    {
        return GetStream().Rewind(n);
    }
    bool empty() const requires requires { GetStream().empty(); }
    {
        return GetStream().empty();
    }
};
