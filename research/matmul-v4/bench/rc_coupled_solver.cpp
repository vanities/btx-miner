// v4.5 ENC_RC COUPLED puzzle -- byte-exact CPU reference (matador side).
//
// Reproduces btx PR#89's frozen coupled toy golden 7a7ce106... for MakeCoupHeader(42) @ height 0,
// with the DATACENTER-DEFAULT levers numair flipped ON in 1ea2a63:
//   full_bank_schedule = TRUE, material_exchange = TRUE, mode = SequentialLobes.
//
...
// btx build required.
//
//   c++ -O2 -std=c++17 rc_coupled_solver.cpp -o rc_coupled_solver && ./rc_coupled_solver
//   EXPECT: 7a7ce1065c7881aa2bd2295c26778ebf88c22432e91326f98d098c11885579ee
...
static constexpr char kBankTag[]="BTX_RC_COUP_BANK_V1", kLobeTag[]="BTX_RC_COUP_LOBE_V1",
                      kBarrierTag[]="BTX_RC_COUP_BARRIER_V1", kPermTag[]="BTX_RC_COUP_PERM_V1",
                      kMixTag[]="BTX_RC_COUP_MIX_V1", kExtractTag[]="BTX_RC_COUP_EXTRACT_V1",
                      kFullBankTag[]="BTX_RC_COUP_FULL_BANK_V1",
                      kMatXchgTag[]="BTX_RC_COUP_MAT_XCHG_V1", kEpisodeTag[]="BTX_RC_COUP_EPISODE_V1";
...
// ---- coupled toy params --------------------------------------------------------------------
struct Coup { u32 barriers=4, lobes=4, lobe_width=32, bank_pages=8;
              u32 state_bytes() const { return lobes*lobe_width; } };
static constexpr u32 kPagesPerBL = 12;   // dc::kRCCoupPagesPerBarrierLobe (full schedule)
...
static std::vector<u32> select_pages(u32 barrier, u32 lobe, const H256& sigma, const Coup& p) {
    ...
    const u64 base = (u64(barrier)*p.lobes + lobe)*u64(kPagesPerBL);
    std::vector<u32> out(kPagesPerBL);
    for (u32 k=0;k<kPagesPerBL;++k) out[k]=perm[size_t((base+k)%p.bank_pages)];
    return out;
}

// ---- lobe seeds / init / GEMV --------------------------------------------------------------
...
// LobeLocalGemm: 1xW row . WxW page -> 1xW int64 (coupled.cpp:282)
static void lobe_gemv(const i8* row, const std::vector<i8>& page, u32 W, i64* out) {
    for (u32 c=0;c<W;++c){ i64 acc=0; for(u32 k=0;k<W;++k) acc += i64(row[k])*i64(page[size_t(k)*W+c]);
                           out[c]=acc; }
}
...
static void mix_ascending(std::vector<i64>& s, u32 mask, u32 n) {
    for (u32 stage=0; (u32(1)<<stage)<n; ++stage){ u32 stride=u32(1)<<stage;
        for (u32 i=0;i<n;++i){ u32 j=i^stride; if(i>=j) continue; u32 pi=i^mask, pj=j^mask;
            i64 a=s[pi], b=s[pj]; s[pi]=a+b; s[pj]=a-b; } }
}
static void mix_descending(std::vector<i64>& s, u32 mask, u32 n) {
    ...
    for (int stage=int(bits)-1; stage>=0; --stage){ u32 stride=u32(1)<<u32(stage);
        for (u32 i=0;i<n;++i){ u32 j=i^stride; if(i>=j) continue;
            ...
            i64 a=s[pi], b=s[pj]; s[pi]=a+b; s[pj]=b-a; } }
}
...
static H256 run_coupled(const CoupHeader& h, u32 height) {
    ...
    // nonce-fresh lobe activation: first row of a WxW MX tile per lobe
    const auto lseeds = lobe_seeds(sigma, p);
    std::vector<i8> state(n);
    for (u32 e=0;e<p.lobes;++e){
        const auto tile = expand_mx_dequant_i8(lseeds[e], p.lobe_width, p.lobe_width);
        std::memcpy(state.data()+e*p.lobe_width, tile.data(), p.lobe_width);
    }

    std::vector<H256> roots(p.barriers);
    for (u32 b=0;b<p.barriers;++b){
        std::vector<i64> acc(n, 0);
        // C3.a local GEMM per lobe vs selected bank pages (full schedule)
        for (u32 e=0;e<p.lobes;++e){
            const auto ids = select_pages(b, e, sigma, p);
            i64* dest = acc.data()+e*p.lobe_width;
            for (u32 pid : ids){
                std::vector<i64> partial(p.lobe_width, 0);
                lobe_gemv(state.data()+e*p.lobe_width, pages[pid], p.lobe_width, partial.data());
                for (u32 c=0;c<p.lobe_width;++c) dest[c]+=partial[c];
            }
        }
        // C3.b balanced permutation, C3.c all-to-all mix, C3.d Extract, C3.e feed-forward
...
int main() {
    ...
    static const char* want = "7a7ce1065c7881aa2bd2295c26778ebf88c22432e91326f98d098c11885579ee";
    std::printf("sigma  = %s\n", hex(sigma).c_str());
    std::printf("digest = %s\n", hex(digest).c_str());
    std::printf("EXPECT = %s\n", want);
    const bool ok = hex(digest)==want;
    std::printf("=> ENC_RC COUPLED %s\n", ok ? "BYTE-EXACT (7a7ce106)" : "MISMATCH");
    return ok ? 0 : 1;
}
